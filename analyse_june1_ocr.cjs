...
let outputStr = `=== DETAILED ANALYSIS OF 1ST JUNE OCR FILES (${blocks.length} files) ===\n`;
...
blocks.forEach((b, idx) => {
  ...
  // Print first 10 lines for context
  logLine(`  First 10 lines of text:`);
  lines.slice(0, 10).forEach(l => logLine(`    > ${l}`));
});

fs.writeFileSync("C:\\Users\\Administrator\\.gemini\\antigravity-ide\\scratch\\june1_details_dump_utf8.txt", outputStr, "utf-8");
console.log("Analysis written to C:\\Users\\Administrator\\.gemini\\antigravity-ide\\scratch\\june1_details_dump_utf8.txt");
