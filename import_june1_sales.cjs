...
const staysToImport = [
  { invoiceNo: "11908", guest: "ROBI STEPHAN SS", date: "20260601", amount: 1100.00, rooms: ["Room 19"], mode: "DIRECT" },
  { invoiceNo: "11905", guest: "JAGTAR", date: "20260601", amount: 1500.00, rooms: ["Room 08"], mode: "DIRECT" },
  { invoiceNo: "180962826-000231", guest: "SHARAJ SINGH PADDA", date: "20260601", amount: 1638.00, rooms: ["Room 04"], mode: "TREEBO" },
  { invoiceNo: "11911", guest: "MAYANK SHRIVASTVA", date: "20260601", amount: 3600.00, rooms: ["Room 07", "Room 08"], mode: "DIRECT" },
  { invoiceNo: "11909", guest: "HARDEEP SINGH", date: "20260601", amount: 1200.00, rooms: ["Room 17"], mode: "DIRECT" },
  { invoiceNo: "11912", guest: "TARUN GOYAL", date: "20260601", amount: 1100.00, rooms: ["Room 20"], mode: "DIRECT" },
  { invoiceNo: "11906", guest: "PARSHANT", date: "20260601", amount: 1500.00, rooms: ["Room 11"], mode: "DIRECT" },
  { invoiceNo: "11910", guest: "RAHUL PUROHIT", date: "20260601", amount: 2000.00, rooms: ["Room 10"], mode: "DIRECT" },
  { invoiceNo: "11913", guest: "RAJEEV PARMAR", date: "20260601", amount: 1200.00, rooms: ["Room 16"], mode: "DIRECT" }
];

const foodBillsToImport = [
  { billNo: "16028", guest: "SHARAJ SINGH PADDA", date: "20260601", total: 60.00, rooms: ["Room 04"] },
  { billNo: "16024", guest: "MAYANK SHRIVASTVA", date: "20260601", total: 530.00, rooms: ["Room 07", "Room 08"] },
  { billNo: "16022", guest: "VARUN", date: "20260601", total: 233.00, rooms: ["Room 02"] },
  { billNo: "16033", guest: "HARDEEP SINGH", date: "20260601", total: 100.00, rooms: ["Room 17"] },
  { billNo: "16025", guest: "PARSHANT", date: "20260601", total: 591.00, rooms: ["Room 11"] },
  { billNo: "16027", guest: "BHARAT SIR", date: "20260601", total: 705.00, rooms: [] },
  { billNo: "16021", guest: "A&A STAFF", date: "20260601", total: 90.00, rooms: [] },
  { billNo: "16020", guest: "ENGLABS INDIA PVT LTD", date: "20260601", total: 525.00, rooms: [] },
  { billNo: "16018", guest: "MD SIR", date: "20260601", total: 445.00, rooms: [] },
  { billNo: "16019", guest: "Bright Kids School", date: "20260601", total: 50.00, rooms: [] }
];
...
async function run() {
  const directors = ["MD SIR", "BHARAT SIR", "SWATI MAAM", "SHREEYA", "A&A STAFF", "Bright Kids School", "ENGLABS INDIA PVT LTD"];
...
<ENVELOPE>
  ...
  console.log("\n=== STEP 2: Creating Cost Categories & Cost Centres ===");
  const costCentres = [
    "Room 04", "Room 07", "Room 08", "Room 10", "Room 11", "Room 16", "Room 17", "Room 19", "Room 20", "Room 02"
  ];
  const costSetupXmls = [
...
<ENVELOPE>
  ...
  console.log("  Cost Centres configuration done.");

  console.log("\n=== STEP 3: Ensuring Stock Items Exist ===");
  const stockItemsToEnsure = [
  ...
  for (const item of stockItemsToEnsure) {
...
<ENVELOPE>
    ...
    try {
      ...
      if (res.includes("<CREATED>1</CREATED>") || res.includes("<ALTERED>1</ALTERED>")) {
        console.log(`  SUCCESS: Ensured StockItem '${item.name}' exists.`);
      } else {
        console.log(`  INFO: StockItem '${item.name}' response: ${res.trim()}`);
      }
    } catch (e) {
      console.error(`  ERROR creating stock item '${item.name}': ${e.message}`);
    }
  }

  console.log("\n=== STEP 4: Creating Guest Ledgers ===");
  ...
  for (const guest of guestLedgerList) {
    const parentGroup = guest.mode === "DIRECT" ? "Hotel Guests - Direct" 
                      ...
                      : "Customer"; // Directors go to Customer group
...
<ENVELOPE>
    ...
    try {
      ...
      if (res.includes("<CREATED>1</CREATED>")) {
        ...
        // Let's also send an ALTER message to make sure ISBILLWISEON = No and parent is correct
        const alterXml = `
<ENVELOPE>
    <HEADER><TALLYREQUEST>Import Data</TALLYREQUEST></HEADER>
    <BODY>
        ...
        console.log(`  WARNING: Guest ledger '${guest.name}' response: ${res.trim()}`);
      }
    } catch (e) {
...
<ENVELOPE>
    ...
    try {
      const res = await postTallyXml(xml);
      if (res.includes("<CREATED>1</CREATED>")) {
        console.log(`  SUCCESS: Room Stay Invoice ${s.invoiceNo} (${s.guest}) posted. Amt: ${s.amount}`);
      } else {
  ...
  for (const fb of foodBillsToImport) {
    ...
    items.forEach(i => {
      ...
      } else if (directors.includes(fb.guest)) {
        costCentreXml = `
                                <CATEGORYALLOCATIONS.LIST>
